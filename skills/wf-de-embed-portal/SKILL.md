...
**Sub-skills used:**
- `cap-de-programmatic-filters` — Read-only embed token flow, server-side filters, dataset switching
- `cap-de-edit-embed` — Edit embed via Identity Broker, JWT auth, Domo roles
- `cap-de-jsapi-filters` — Client-side filtering, drill events, iframe communication
...
> **Sub-skill handoff — required, even for focused questions.** The OAuth → embed token flow is owned entirely by `cap-de-programmatic-filters`. Implementing it here would duplicate a maintained skill and create a second source of truth that drifts over time. The sub-skill has the complete implementation including edge cases, token size limits, SQL filters, and dataset switching that are easy to miss. Users who skip it and follow an inline version often hit production issues. Your response must explicitly name `cap-de-programmatic-filters` and provide only the portal wiring below.

In your response, tell the user:

> "For the embed token flow (OAuth → access token → embed token → iframe), follow the **`cap-de-programmatic-filters`** skill. Here's the portal-specific wiring that connects your user data to it:"
...
// OAuth + embed token implementation: follow cap-de-programmatic-filters
...
export async function POST(req: NextRequest) {
  ...
  // 3. Hand off to cap-de-programmatic-filters with these portal values:
...
export async function POST(req: NextRequest) {
  ...
  // 2. Hand off to cap-de-edit-embed with these portal values:
  ...
  // cap-de-edit-embed handles jwt.sign(), IDP URL construction, and iframe rendering.
...
> **Sub-skill handoff — required.** The MessagePort setup, filter API, and event handling are owned entirely by `cap-de-jsapi-filters`. Implementing them here would duplicate that skill. Your job is only the portal mounting pattern.

In your response, tell the user:

> "For the JS API (MessagePort setup, filter application, drill events, iframe resize), follow the **`cap-de-jsapi-filters`** skill. Here's how to mount it in the portal:"

Then provide **only** the mounting pattern:

```tsx
// app/layout.tsx — mount before any Domo iframe loads
// Full JS API implementation: follow cap-de-jsapi-filters
...
Common portal patterns to mention (cap-de-jsapi-filters implements all of these):
...
- **Read-only branch**: follows `cap-de-programmatic-filters` (POST form → iframe)
- **Edit branch**: follows `cap-de-edit-embed` (direct iframe `src` → Identity Broker URL)
...
// Read-only rendering: follow cap-de-programmatic-filters
// Edit rendering: follow cap-de-edit-embed
```
