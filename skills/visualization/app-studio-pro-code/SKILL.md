...
### DEFAULT VISUALIZATION STRATEGY (apply whenever pro-code charts are built)

When pro-code components are added to an App Studio app, apply this two-tier default pattern across all pages. Do not deviate unless the user explicitly requests a different chart type.

#### Overview / Main Page — Multi-Line ComposedChart

The main/overview page chart is always a **time-based `ComposedChart`** with multiple line series:

| Series | Line Style | Color Role | Data Meaning |
|--------|-----------|-----------|-------------|
| Actuals | Solid, `strokeWidth: 2` | `COLORS.primary` | Historical measured values |
| Plan / Prior Period | Solid, `strokeWidth: 1.5`, slight opacity | `COLORS.secondary` | Budget, target, or same-period-last-year |
| Forecast | Dashed (`strokeDasharray: '4 3'`), `strokeWidth: 2` | `COLORS.secondary` | Model prediction beyond "today" |
| Confidence Band | `Area` with gradient fill, no stroke | `COLORS.bandTop`/`COLORS.bandBot` | Upper/lower MAE or prediction interval |

**Always include all of the following controls and elements:**
- **Day / Week / Month toggle** (`<select>` with `aggregateData(rawData, period)`) — see aggregation pattern below
- **Confidence band toggle button** (`showBand` state, hidden when dataset has no bounds columns)
- **"Today" `ReferenceLine`** — dashed vertical line at current date, only rendered when today falls within the data range
- **Custom `ChartTooltip`** — shows date header + all series values at hover point + confidence bounds
- **Manual footer legend** — do NOT use Recharts built-in legend (hide it with `.recharts-legend-wrapper { display: none !important }`)

**Required Recharts imports for main page:**
```javascript
import { ComposedChart, Area, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine } from 'recharts';
```

**Required `COLORS` object — customize hex values from the project's DESIGN.md:**
```javascript
const COLORS = {
  primary:   '#3A6983',  // actuals line — project palette brand hex
  secondary: '#5E92CE',  // plan/prior period and forecast line
  muted:     '#99A9BD',  // confidence bound labels in tooltip
  bandTop:   '#B2D5F3',  // confidence gradient top (opacity 0.4)
  bandBot:   '#CAE1F0',  // confidence gradient bottom (opacity 0.15)
  grid:      '#E5E7EB',  // CartesianGrid stroke
  axisLine:  '#E5E7EB',  // XAxis axisLine stroke
  text:      '#111827',  // chart title, tooltip header
  textSec:   '#6B7280',  // subtitle, axis ticks, Y-label
  today:     '#9CA3AF',  // ReferenceLine stroke (dashed)
  todayLabel:'#6B7280',  // ReferenceLine label fill
};
```

**Reference implementation**: `forecast line recharts/app.js` — production-verified, 482 lines. The full template code is in the "Proven Reference Pattern 1" section below.

---

#### Sub-Pages — Time-Based Bar Chart (Default)

Every sub-page chart is a **time-based `BarChart`** — time on the x-axis, primary metric on y-axis, aggregated by the same day/week/month toggle. Do NOT default to category-axis bar charts on sub-pages unless the page data has no meaningful time dimension.

| Property | Value |
|----------|-------|
| x-axis | Date/time (same `DATE_COL` pattern as main page) |
| y-axis | Page-specific metric |
| Aggregation | Day/week/month toggle (same `aggregateData` function) |
| Bar radius | `[2, 2, 0, 0]` (rounded top) |
| Max bar size | `40` |
| Grid | Horizontal only (`vertical: false`) |
| Today line | `ReferenceLine` (same as main page) |
| Tooltip | Same `ChartTooltip` component |

```javascript
// Sub-page: time-based BarChart — use this as the starting template
...
React.createElement(ResponsiveContainer, { width: '100%', height: '100%' },
  React.createElement(BarChart, { data: chartData, margin: { top: 10, right: 20, left: 10, bottom: 20 } },
    ...
    React.createElement(Bar, {
      dataKey: VAL_COLS[0],
      fill: COLORS.primary, radius: [2, 2, 0, 0], maxBarSize: 40
    })
  )
)
```

**When to use a different chart type**: Only deviate from time-based bar to a category, stacked, horizontal, or scatter chart when the page's primary data question is NOT time-trend-based (e.g., "top 10 regions by revenue" → horizontal bar with category axis). See the "Alternative Chart Type Templates" section below for those patterns.

---

#### Aggregation Function (Shared Across All Pages)

Use a single `aggregateData` function in every pro-code chart component. It groups by day/week/month and sums numeric values:
...
Force a chart re-mount when aggregation changes by keying the wrapper div: `key={\`w-${aggregation}\`}`.
...
**Default: Overview page uses multi-line ComposedChart; sub-pages use time-based BarChart.** See the "DEFAULT VISUALIZATION STRATEGY" section above for full details. Only use alternative chart types when the sub-page's primary data question is NOT time-trend-based. Select chart type based on the page's data shape:
